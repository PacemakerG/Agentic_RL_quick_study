...
project_name=MedAgent-RL-Smoke
exp_name=medagent-rl-grpo-smoke-8x24gb-${run_stamp}
...
# 8 个病例、每个病例 4 条 GRPO 轨迹；全局 micro batch=8，归一化后每卡为 1。
# Smoke 只跑 1 个训练 step、环境与外层都限制为 2 轮，用于验证完整 RL 数据流和显存是否正常。
# GRPO 不启动 Critic；关闭 KL 后也不启动无用的 Reference Policy。
# 策略模型参数卸载到 CPU，Patient Simulator 和 Adam optimizer 留在 GPU，平衡 Host RAM 与显存占用。
# Qwen2.5-7B 有 28 个 attention heads，因此 tensor parallel 必须使用能整除 28 的 4，不能使用 8。
...
"$python_bin" -m ragen.trainer.main_ppo \
  ...
  env.env_llm.vllm_config.max_num_seqs=32 \
