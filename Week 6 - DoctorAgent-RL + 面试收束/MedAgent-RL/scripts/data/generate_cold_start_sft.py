#!/usr/bin/env python3
"""按病例生成问诊策略的思考过程，并组装最终的 SFT JSON。

模型接收一个完整病例，仅返回：

    {"thinkings": [{"turn_id": 1, "thinking": "..."}]}

原始 prompt 和策略回复由本地程序组装，最终每条数据只包含
prompt 和 response。
"""
...
MODEL_NAME = os.environ.get("API_MODEL_NAME", "deepseek-v3")
...
THINKING_SYSTEM_PROMPT = """You generate concise medical reasoning for consultation-policy turns in a multi-turn consultation.

Requirements:
1. Return JSON only. Do not use Markdown fences.
2. Return exactly one thinking item for every policy turn_id.
3. Do not rewrite, quote, or return the policy answer.
4. For each policy turn, reason only from information available before that turn. Never use later dialogue turns.
5. The thinking should explain why that question, diagnosis, or recommendation is the appropriate next action.
6. Use the same language as the corresponding policy message.
