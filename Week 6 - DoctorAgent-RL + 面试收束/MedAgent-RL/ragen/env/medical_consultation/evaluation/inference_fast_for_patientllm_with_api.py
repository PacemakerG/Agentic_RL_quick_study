...
import re
import argparse
# We will not use AutoModelForCausalLM and AutoTokenizer for the consultation policy
...
class MedicalDialogueSimulation:
    def __init__(self, policy_model_name, input_file, output_file, temperature=0.7, top_p=0.9, patient_model_path="Qwen2.5-7B-Instruct", device_map="auto", batch_size=8):
        """
        初始化医患对话模拟系统
        Args:
            policy_model_name (str): 医生模型名称 (DeepSeek-v3 model name).
        ...
        self.policy_model_name = policy_model_name # DeepSeek-v3 model name
        ...
        if "gpt" in policy_model_name.lower():
        ...
        elif "qwen" in policy_model_name.lower():
            self.doctor_client = OpenAI(
                base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",
                api_key=api_key,
            )
        elif "llama" in policy_model_name.lower():
            self.doctor_client = OpenAI(
        ...
        print(f"Initialized API client for model: {self.policy_model_name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Medical Dialogue Simulation with DeepSeek-v3 API (Consultation Policy) and Local Model (Patient)")
    parser.add_argument("--policy_model_name", type=str, default="deepseek-v3-250324",
    ...
    parser.add_argument("--temperature", type=float, default=0.7,
                        help="Temperature for consultation policy generation")
    parser.add_argument("--top_p", type=float, default=0.9,
                        help="Top-p sampling parameter for consultation policy")
    ...
    parser.epilog = """
    Example usage:
    # Run simulation with DeepSeek-v3 as consultation policy and Qwen as patient
    # Ensure API_KEY environment variable is set
    python your_script_name.py --policy_model_name deepseek-v3-250324 --patient_model_path /path/to/qwen_model --input_file input.json --output_dir ./results --add_timestamp --batch_size 16
    ...
    if args.verbose:
        print(f"Starting Medical Dialogue Simulation with DeepSeek-v3 API (Consultation Policy) and Local Model (Patient)")
        print(f"Consultation policy: {args.policy_model_name}")
