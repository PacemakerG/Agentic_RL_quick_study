    ...
    for candidate, reference in data_pairs:
        ...
        if info_gather:
            with open('ragen/env/medical_consultation/evaluation/eval_information_prompt_template.txt', 'r') as file:
                prompt = file.read()
                prompt = prompt.format(patient_self_report=reference, policy_gathered_info=candidate)
        else:
...
def calculate_category_averages(results_by_category):
    ...
    for category, results in results_by_category.items():
        ...
        avg_result = {
            ...
            "protocol_quality": {
                "repetition_rate": np.mean([r["protocol_quality"]["repetition_rate"] for r in results]),
                "format_error_rate": np.mean([r["protocol_quality"]["format_error"] for r in results]),
                "completion_rate": np.mean([r["protocol_quality"]["completed"] for r in results])
            },
...
def evaluate_all_cases(simulation_data_list, reference_data_list=None, alpha=1.0, beta=0.01, batch_size=16):
    ...
    for i in tqdm(range(len(simulation_data_list)), desc=f"Process {global_rank if global_rank is not None else 0}: Calculating Other Metrics"):
        ...
        model_diagnosis, model_recommendation, _ = evaluator.extract_model_outputs()

        policy_questions = [
            " ".join(turn.get("content", "").lower().split())
            for turn in sim_item.get("simulation_dialogue", [])
            if turn.get("role") == "doctor" and not turn.get("is_diagnosis", False) and turn.get("content")
        ]
        repeated_questions = len(policy_questions) - len(set(policy_questions))
        repetition_rate = repeated_questions / len(policy_questions) if policy_questions else 0.0
        format_error = float(not model_diagnosis or not model_recommendation)
        completed = float(bool(sim_item.get("is_completed", False)))
    ...
    avg_result = {
        ...
        "protocol_quality": {
            "repetition_rate": np.mean([r["protocol_quality"]["repetition_rate"] for r in case_results]),
            "format_error_rate": np.mean([r["protocol_quality"]["format_error"] for r in case_results]),
            "completion_rate": np.mean([r["protocol_quality"]["completed"] for r in case_results])
        },
...
def main():
    ...
    parser.add_argument('--simulation_data', type=str,
                        default='outputs/evaluation/simulation_results.json',
                        help='Path to simulation data JSON file')
    parser.add_argument('--reference_data', type=str,
                        default='data/MTMedDialog_test.json',
    ...
    if results:
        ...
        if global_rank is None or global_rank == 0:
            ...
            if "average_result" in results and results["average_result"]:
                ...
                print(f"Repetition Rate: {avg['protocol_quality']['repetition_rate']:.4f}")
                print(f"Format Error Rate: {avg['protocol_quality']['format_error_rate']:.4f}")
                print(f"Completion Rate: {avg['protocol_quality']['completion_rate']:.4f}")
