...
def get_train_command(config: Dict[str, Any]) -> str:
    ...
    if config['rl_or_sft'] != "rl":
        raise ValueError("This launcher only supports the curated GRPO workflow; use scripts/train_sft_8gpu.sh for SFT.")
...
def main():
    parser = argparse.ArgumentParser(description='Launch MedAgent-RL training from YAML config')
    parser.add_argument(
        'env_name',
        nargs='?',
        default='medical_consultation',
        help='Medical consultation environment config name',
    )
